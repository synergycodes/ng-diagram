...
- **`NgDiagramService.invalidateMeasurements` is awaitable** — it now returns a `Promise<void>` that resolves once the triggered re-measurements have settled, so the invalidated elements' `size`, `measuredPorts` and `measuredLabels` read fresh on the next line. This replaces the empty-transaction workaround (`await transaction(() => {}, { waitForMeasurements: true })`) that was previously the only way to await an invalidation. Resolution uses the same settle mechanism as the transaction `waitForMeasurements` option, so an invalidated element that never delivers a new measurement (unmounted, zero-size, or simply unchanged) does not stall the promise. Existing call sites keep working (`void` → `Promise<void>`); ignoring the promise is still fine ([#773](https://github.com/synergycodes/ng-diagram/pull/773))
...
- **`waitForMeasurements` no longer resolves early when measurement rounds overlap** — a debounce timer left over from a previous round could fire during a newly started one and settle it before its measurements arrived; the new round now clears it ([#773](https://github.com/synergycodes/ng-diagram/pull/773))
