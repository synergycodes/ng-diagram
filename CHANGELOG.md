...
- **`deleteNodes` on a group deletes its whole subtree** — `NgDiagramModelService.deleteNodes` now cascades to all descendants of the deleted nodes, matching `deleteSelection`. Previously the group's children survived with a dangling `groupId` pointing at the deleted parent, and children of a hidden group popped back into view as orphans
