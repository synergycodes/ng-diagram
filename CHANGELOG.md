...
- `NgDiagramPaletteItemPreviewComponent` protected members `isChrome`, `isSafari`, `isVisible` and `scale` — the preview no longer branches per browser or toggles visibility, and the viewport scale is applied internally. They keep working for subclasses and will be removed in the next major version ([#801](https://github.com/synergycodes/ng-diagram/pull/801))

### Changed

- **`ng-diagram-palette-item-preview` clips its content internally** — the preview host renders as a plain zero-height block, and the off-screen copy it holds is clipped by an internal wrapper, so it stays contained whatever the surrounding app does — application styles on the host cannot unclip it. The host takes no vertical space and paints nothing, but it is a block: inline content placed next to it inside a palette item now starts on a new line ([#801](https://github.com/synergycodes/ng-diagram/pull/801))
- **`NgDiagramPaletteItemPreviewComponent.preview` points at the parked copy** — the element behind this signal is the off-screen, clipped copy of the preview content. Read the preview's natural size or content from it; its position on the page is meaningless ([#801](https://github.com/synergycodes/ng-diagram/pull/801))
- **The drag image zoom is capped at 3x** — at higher zoom levels the image under the cursor stays at 3x the preview's natural size, so starting a drag stays smooth even on large previews; the dropped node still lands at the current zoom ([#801](https://github.com/synergycodes/ng-diagram/pull/801))
