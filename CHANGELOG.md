...
- **Pasting an edge no longer duplicates the connection between the original nodes** — a copied edge whose endpoint nodes were left behind (including the one-endpoint-copied case) used to be pasted attached to the _original_ nodes, stacking an identical connection on top of the existing one with every paste. Such endpoints now become dangling instead — empty `source`/`target` with an authored `sourcePosition`/`targetPosition` at the edge's last attachment point — so the pasted edge keeps its shape and can be reconnected or repositioned. Endpoints whose node was copied together with the edge are remapped to the pasted nodes as before; already-dangling endpoints stay dangling; free endpoints anchor the pasted content at the paste position exactly like node positions do, so pasting only edges centers them at the cursor. An edge whose freed endpoint has no known position (never routed) is skipped, and a paste that would add nothing leaves the current selection untouched ([#XXX](https://github.com/synergycodes/ng-diagram/pull/XXX))
