...
- **`waitForMeasurements` no longer resolves early when measurement rounds overlap** — a leftover debounce timer from a previous round could settle a newly started round before its measurements arrived ([#773](https://github.com/synergycodes/ng-diagram/pull/773))
...
- **`waitForMeasurements` accuracy** — a concurrent unrelated update can no longer make `waitForMeasurements` resolve too early or wait for the wrong elements; the option passed to a nested transaction now logs a warning instead of silently resolving before the updates are applied ([#769](https://github.com/synergycodes/ng-diagram/pull/769))
...
- **`NgDiagramNodeService.resizeNode` now resizes an unselected group** — a programmatic `resizeNode` call targeting a group node used to silently do nothing unless the group was selected; it now applies regardless of selection state, still enforcing the group resize constraints (children containment, minimum node size and resize snapping). Interactive resize behavior is unchanged. Calls that used to silently no-op now apply; there is no signature change ([#772](https://github.com/synergycodes/ng-diagram/pull/772))
