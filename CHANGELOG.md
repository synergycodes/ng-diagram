...
- **The cursor no longer flickers while an edge is drawn or relinked** — the preview edge does not react to the pointer, whatever template draws it, and the diagram shows the grabbing hand over the canvas for the whole gesture. The edge wrapper carries an `ng-diagram-edge--temporary` class, so interactive content in your own edge template can stay out of the way during a draw ([#812](https://github.com/synergycodes/ng-diagram/pull/812))
