...
- **`NgDiagramDefaultEdgeLabelComponent` is now public** — the component rendering the default edge label chip (`ng-diagram-default-edge-label`) is exported from the package, so custom edge templates can compose it inside `ng-diagram-base-edge-label` and get the default label look (theme-aware chip, selected-state border) without copying its styles. The selected-state border now works in any edge template: the component styles its own selected state, using `--edge-label-border-color` when set and falling back to the `--ngd-default-edge-stroke-selected` token. The class was previously internal (named `DefaultEdgeLabelComponent`, never exported), so the rename is not a breaking change ([#XXX](https://github.com/synergycodes/ng-diagram/pull/XXX))
