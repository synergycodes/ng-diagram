...
- **Awaitable service methods** — every mutating method on the public services (`NgDiagramModelService`, `NgDiagramNodeService`, `NgDiagramSelectionService`, `NgDiagramClipboardService`, `NgDiagramGroupsService`, `NgDiagramViewportService`) now returns a `Promise<void>` that resolves only after the middleware chain has run and the change has been committed to the model. `CommandHandler.emit` resolves at the same point, and commands await their nested emits. Call sites are backward compatible (`void` → `Promise<void>`). Note: Angular signals (`NgDiagramModelService.nodes()` etc.) still refresh asynchronously after the promise resolves. **Do not await these promises from inside a middleware** — the middleware chain runs inside the update lock and the await would deadlock; fire-and-forget calls queue safely ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))
...
- `waitForMeasurements` option on content-creating and updating service methods — `addNodes`, `addEdges`, `updateNode`, `updateNodes`, `updateNodeData`, `updateEdge`, `updateEdges`, `updateEdgeData` on `NgDiagramModelService`, `resizeNode` on `NgDiagramNodeService` and `paste` on `NgDiagramClipboardService` accept `options?: { waitForMeasurements?: boolean }`; when set, the returned promise resolves only after measurements triggered by the change have completed, so `zoomToFit()`/`centerOnNode()` can be called immediately after. Inside an already active transaction the option is ignored with a console warning — pass `{ waitForMeasurements: true }` to the transaction itself instead ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))

- **`NgDiagramService.transaction` always returns the commit promise** — the synchronous-callback overload returned `void`, and async-callback detection relied on a realm-sensitive `instanceof Promise` check that silently discarded the commit promise for async callbacks created outside the Zone.js-patched realm (cross-realm callers, e2e drivers, iframes); all overloads now return `Promise<TransactionResult>` ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))

### Fixed

- **Gesture lifecycle hardening** — now that command emits genuinely await the state commit (and can reject), the gesture flows were hardened end to end: the node-drag handler no longer loses or double-applies movement when emits suspend on asynchronous middleware; a fast re-drag/re-resize/re-rotate no longer gets its freshly started gesture state cleared by the previous gesture's asynchronous cleanup; cleanup — including `moveNodesStop` and group-highlight clearing — runs even when a command emit or user config callback rejects; `finishLinking` and `finishLinkingToPosition` clear the linking state even when connection validation or edge building throws, and only their own gesture's state — a linking gesture started programmatically while the previous finish was still in flight is no longer wiped by it; a drag move that resumed after its gesture's pointerup no longer shifts nodes post-drop, even while the drop itself is still committing; `nodeDragStarted`/`nodeDragEnded`, `nodeResizeStarted`/`nodeResizeEnded` and `nodeRotateStarted`/`nodeRotateEnded` report the node(s) of the gesture that emitted them — a fast re-drag or re-grab no longer swallows or mis-attributes the previous gesture's end event; a finished drop no longer clears a group highlight that a newer gesture has just set; and destroying a gesture's handle mid-drag (e.g. deleting the node while resizing, rotating, or linking) clears its action state instead of leaking it — a leaked resize state suppressed all node measurements and a leaked linking state permanently blocked new links ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))
- **A throwing middleware no longer freezes the diagram** — an uncaught error thrown (or an unhandled rejection) inside a middleware used to leave the in-flight update suspended forever with the update lock held, silently freezing every subsequent update; it now rejects the update (the promise returned by the mutating call rejects) and the diagram keeps working. Middlewares that call `next()` fire-and-forget do not surface unhandled rejections when a later middleware fails the pass. A middleware suspended across a failed or cancelled update no longer resumes the remaining middleware chain against that dead update (which could hijack a concurrent `waitForMeasurements` and emit phantom events), and an error raised after the update already settled — or a distinct error raised while it is failing — is reported via a console error instead of vanishing silently ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))
- **Initialization resilience** — a rejected `init` command pass (e.g. a throwing custom `ModelAdapter`) no longer wedges the diagram in initialization mode swallowing every future measurement; initialization completes with a console error instead ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))
- **`waitForMeasurements` accuracy** — the measurement-tracking request is now staged inside the transaction's own update pass (under the update lock), so a concurrent unrelated update can no longer consume it and register the wrong participants; a pass cancelled by a middleware clears the staged request instead of poisoning the next pass; and `waitForMeasurements` passed to a nested transaction now logs a warning instead of silently resolving before the updates are applied ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))
- **Transaction overlap integrity** — overlapping (fire-and-forget) transactions can no longer silently discard updates: a completing transaction removes itself from the transaction stack by identity instead of positionally, a root transaction's committed result is applied directly instead of being re-queued into an unrelated transaction that opened meanwhile, and a nested transaction that outlives its parent commits its updates independently with a console warning instead of merging them into the dead parent ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))
- **Batched port/label flushes are serialized and error-isolated** — a later flush generation can no longer start before the previous one finished (preventing out-of-order add/delete application for the same port or label under virtualization), and a rejecting flush callback no longer discards the rest of its generation's already-collected updates ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))
