...
- **`tabbable` input on `<ng-diagram>`** — controls whether the diagram container takes part in the page's sequential Tab order. With `[tabbable]="false"` the container element renders with `tabindex="-1"` and the watermark link is skipped by Tab as well, so an application that manages its own Tab order (for example a roving tabindex on the nodes) gets no extra Tab stops from the diagram itself. Focusable content in your own node and edge templates is not affected and keeps its own Tab stops. Both elements stay clickable, stay in the accessibility tree, and keyboard shortcuts keep working whenever focus is inside the diagram. Defaults to `true`, which keeps the current behavior. If you removed the Tab stop by writing a `tabindex` attribute on `<ng-diagram>` yourself, switch to `[tabbable]="false"` — the diagram sets that attribute itself now, so a value written in your template no longer takes effect ([#790](https://github.com/synergycodes/ng-diagram/issues/790), [#795](https://github.com/synergycodes/ng-diagram/pull/795)) — thanks [@jimmeryn](https://github.com/jimmeryn) for the issue submission! 🙏
