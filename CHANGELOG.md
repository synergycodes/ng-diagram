...
- `NgDiagramPaletteItemPreviewComponent.scaleTransform` — the drag image is scaled internally, so this value is no longer used. It will be removed in the next major version ([#NNN](https://github.com/synergycodes/ng-diagram/pull/NNN))
- `NgDiagramPaletteItemPreviewComponent` protected members `isChrome`, `isSafari`, `isVisible` and `scale` — the preview no longer branches per browser or toggles visibility, and the viewport scale is applied internally. They keep working for subclasses and will be removed in the next major version ([#NNN](https://github.com/synergycodes/ng-diagram/pull/NNN))

### Changed

- **`ng-diagram-palette-item-preview` clips its content internally** — the preview host renders as a plain zero-height block, and the off-screen copy it holds is clipped by an internal wrapper, so it stays contained whatever the surrounding app does — application styles on the host cannot unclip it. The host takes no vertical space and paints nothing, but it is a block: inline content placed next to it inside a palette item now starts on a new line ([#NNN](https://github.com/synergycodes/ng-diagram/pull/NNN))
- **`NgDiagramPaletteItemPreviewComponent.preview` points at the parked copy** — the element behind this signal is the off-screen, clipped copy of the preview content. Read the preview's natural size or content from it; its position on the page is meaningless ([#NNN](https://github.com/synergycodes/ng-diagram/pull/NNN))
- **The drag image zoom is capped at 3x** — at higher zoom levels the image under the cursor stays at 3x the preview's natural size, so starting a drag stays smooth even on large previews; the dropped node still lands at the current zoom ([#NNN](https://github.com/synergycodes/ng-diagram/pull/NNN))
...
- **Palette preview no longer bleeds over the palette or stretches the page** — the off-screen copy of each `ng-diagram-palette-item-preview` is kept at its natural size and clipped internally, so at high zoom levels it no longer paints over the palette items, and it no longer adds scrollbars to the page in apps whose palette panel has no positioned ancestor ([#NNN](https://github.com/synergycodes/ng-diagram/pull/NNN))
- **Palette drag image matches the preview it shows** — the image shown under the cursor while dragging a palette item is now exactly the size of the scaled preview, instead of being built from a zero-sized element whose content sat outside it. Wide previews keep their natural line breaks and stay off-screen. Browsers without CSS `zoom` support (Firefox before 126) get the same image built with a transform, so the drag image behaves the same everywhere. If `setDragImage` throws or the tab is hidden, the temporary clone is still cleaned up ([#NNN](https://github.com/synergycodes/ng-diagram/pull/NNN))
