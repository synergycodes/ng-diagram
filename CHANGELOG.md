...
- **Size changes made right after a resize gesture are no longer overwritten by a stale measurement** — when the mouse button was released while still moving, a measurement of the resized node captured during the last gesture frames could be applied after the gesture ended and overwrite a size written afterwards — for example, a middleware reverting an invalid resize on `resizeNodeStop`, or a `nodeResizeEnded` handler correcting the size, only worked when the user paused before releasing the button. Measurements are now stamped at capture time with the node being resized: a gesture-era measurement of that node is never applied after the gesture, the node is re-measured once afterwards (so a size the DOM cannot reach — for example a CSS `min-width` — still reconciles into the model), measurements of other nodes are unaffected, and within one measurement batch only the newest entry per element is applied ([#771](https://github.com/synergycodes/ng-diagram/discussions/771)) — thanks [@logan-brd](https://github.com/logan-brd) for the report and the reproduction! 🙏
