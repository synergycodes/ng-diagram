...
- **Configurable resize edges** — `ng-diagram-node-resize-adornment` accepts a `resizeEdges` input that limits which edges can be grabbed, for example `[resizeEdges]="['right', 'bottom']"` for a node anchored at its top-left corner. All four lines keep rendering (they double as the selection frame), but the ones left out are inert and show no resize cursor, and a corner handle renders only when both of its edges are listed. Omitting the input keeps all four edges and corners active, so existing templates are unaffected ([#775](https://github.com/synergycodes/ng-diagram/pull/775))
