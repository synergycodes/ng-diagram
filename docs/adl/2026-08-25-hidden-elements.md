...
**Registry callbacks are coalesced and deferred**: FlowCore batches all registry callbacks landing in one microtask tick into a single flush — one measurement-expectation prune plus (for node/edge changes) one `templateVisibilityChange` pass. This keeps bulk template-driven toggles at one pass per change-detection cycle instead of one per element, and it moves the whole chain (`refreshHiddenEntities()` → `tryFinish()` → `finish()` → `setState()`) out of any Angular reactive context — a registry write inside an effect can no longer reach a signal write synchronously (NG0600 on Angular 18; the peer deps allow `>=18` while tests run on 19.x). Registry writes therefore need no `untracked` for safety; `untracked` in the writers exists only for dependency scoping. Coalescing is safe because the hidden-computation middleware reads the registry live at execution time, a write landing while the flush's pass is still in flight re-arms exactly one follow-up flush, and a destroyed FlowCore ignores its queued flush. The synchronous registry _state_ (read by the measurement filter) is unaffected — only the callbacks defer. Even with coalescing, bulk hiding should still prefer the model `hidden` flag (one `updateNodes` call = one pass, and it works under virtualization).
