...
Here's an example of a custom model that persists all data directly to localStorage without keeping local copies:

```typescript
import { ModelAdapter, Node, Edge, Metadata } from '@angularflow/angular-adapter';

export class LocalStorageModelAdapter implements ModelAdapter {
  private callbacks: Array<(data: { nodes: Node[]; edges: Edge[]; metadata: Metadata }) => void> = [];
  private storageKey: string;

  constructor(storageKey: string = 'ng-diagram-data') {
  ...
  setMetadata(next: Metadata | ((prev: Metadata) => Metadata)): void {
    ...
    const newMetadata = typeof next === 'function' ? next(currentMetadata) : next;
  ...
  unregisterOnChange(callback: (data: { nodes: Node[]; edges: Edge[]; metadata: Metadata }) => void): void {
  ...
  private getStorageData(): { nodes: Node[]; edges: Edge[]; metadata: Metadata } {
    try {
      const stored = localStorage.getItem(this.storageKey);
      return stored ? JSON.parse(stored) : { nodes: [], edges: [], metadata: { viewport: { x: 0, y: 0, scale: 1 } } };
    } catch (error) {
      console.warn('Failed to read from localStorage:', error);
      return { nodes: [], edges: [], metadata: { viewport: { x: 0, y: 0, scale: 1 } } };
    }
  }

  private updateStorageData(updates: Partial<{ nodes: Node[]; edges: Edge[]; metadata: Metadata }>): void {
    try {
...
### Using Custom Model Implementations

Once you have a custom model implementation, you can use it directly with the `NgDiagramComponent`:
...
@Component({
  ...
  template: `
    <ng-diagram-context>
      <ng-diagram [model]="customModel" />
    </ng-diagram-context>
  `,
  providers: [LocalStorageModelAdapter],
})
export class MyDiagramComponent {
  customModel = new LocalStorageModelAdapter('my-diagram-storage');

  constructor() {
    // Initialize with some data
    this.customModel.updateNodes([
      {
        id: '1',
        position: { x: 100, y: 100 },
...
For enterprise applications, you might want to persist diagram data to a database and synchronize changes across multiple users. A database-backed model adapter would implement the `ModelAdapter` interface to communicate with your backend API, handling both read and write operations. This approach enables features like automatic saves, version history, and data validation on the server side.

```typescript
class DatabaseModelAdapter implements ModelAdapter {
  constructor(private apiService: ApiService) {}

  async updateNodes(nodes: Node[]): Promise<void> {
    await this.apiService.saveNodes(nodes);
    // Update local state and notify callbacks
  }

  // Implementation that syncs with your backend
}
```
