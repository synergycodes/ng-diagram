...
sidebar:
  order: 5
helpBanner:
  title: Designing the look and feel of your diagram?
  body: Diagram-heavy applications are part of what our team does day to day. If you want a second pair of eyes on the visual language, interaction model, or accessibility before you commit to an implementation, we can help shape it with you.
---

import { Aside } from '@astrojs/starlight/components';

<Aside type="note">
  If you want to get started quickly, you can skip the detailed customization sections below. The default styling works
  out of the box and provides a professional appearance without any additional configuration.
</Aside>

<Aside type="caution" title="Important">
  This library relies on **global CSS variables**. Make sure the stylesheet is imported in your **global** styles file
  (e.g. `src/styles.scss`) as shown in the [Quick Start](/docs/intro/quick-start).
</Aside>
...
<Aside type="note">
  The actual color values are defined in the library. Override these variables to customize the color palette for your
  design system.
</Aside>
...
<Aside type="note">The customization options above are **optional**</Aside>
...
Directives require access to node/group data. When used outside of node templates, they won't work. For manual control, use CSS classes instead.

### NgDiagramNodeSelectedDirective

Automatically adds selection styling based on the node's [`selected`](/docs/api/types/model/simplenode/#selected) property:
...
- Adds `ng-diagram-node-selected` class when [`selected`](/docs/api/types/model/simplenode/#selected) is `true`
...
- Adds `ng-diagram-group-highlight` class when [`highlighted`](/docs/api/types/model/groupnode/#highlighted) is `true`
...
<Aside type="caution" title="Important">
  Always use `ng-diagram-node-wrapper` together with these classes to ensure proper transition effects.
</Aside>
