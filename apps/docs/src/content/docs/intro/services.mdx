...
<Aside type="tip" title="Waiting for measurements">
  When you need to perform viewport operations (like `zoomToFit()` or `centerOnNode()`) immediately after adding or
  modifying elements, use the [`waitForMeasurements`](/docs/guides/transactions/#waitformeasurements) transaction option
  to ensure the elements are measured before the viewport adjusts.
</Aside>
