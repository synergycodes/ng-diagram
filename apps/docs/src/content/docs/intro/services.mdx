...
<Aside type="tip">
  All ngDiagram services are scoped to each [`provideNgDiagram()`](/docs/api/utilities/providengdiagram) registration,
  allowing multiple diagrams on the same page with completely isolated state and behavior.
</Aside>
...
[API Documentation →](/docs/api/services/ngdiagramservice)
...
[API Documentation →](/docs/api/services/ngdiagrammodelservice)
...
[Awaiting Changes](/docs/guides/state-management/#awaiting-changes).
...
[API Documentation →](/docs/api/services/ngdiagramviewportservice)
...
export class ViewportControlsComponent {
  ...
  // <ng-diagram [model]="model" (click)="onMouseClick($event)" />
...
[API Documentation →](/docs/api/services/ngdiagramselectionservice)
...
[API Documentation →](/docs/api/services/ngdiagramclipboardservice)
...
:::note
When pasting an edge, endpoints whose node was copied together with it are remapped to the newly
pasted nodes. An endpoint whose node was **not** copied becomes dangling — empty `source`/`target`
with an authored position at the edge's last attachment point — instead of being attached to the
original node, so pasting a lone edge never duplicates the connection between the original nodes.
You can reconnect or reposition the dangling end afterwards. Already-dangling endpoints stay
dangling, and free endpoints anchor the pasted content at the paste position exactly like node
positions do — pasting only edges centers them at the cursor.
:::
...
[View API Documentation →](/docs/api/services/ngdiagramnodeservice)
