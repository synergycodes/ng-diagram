...
- **Transaction Support**: Commands can be grouped into [transactions](/docs/internals/transactions) for atomic execution, ensuring data consistency
...
Learn more in the [Middlewares documentation](/docs/internals/middlewares).

### Data Model

The foundation that stores diagram state and provides the data layer:

- **Nodes**: Visual elements with position, data, and behavior that represent entities in your diagram. See [Nodes documentation](/docs/internals/nodes/nodes) for details.
- **Edges**: Connections between nodes with routing and styling that represent relationships. See [Edges documentation](/docs/internals/edges/edges) for details.
- **Metadata**: Viewport state, middleware configurations, and other settings that control diagram behavior

The default Signal Model uses Angular signals for reactive state management, providing excellent performance and real-time updates. For advanced use cases, custom model adapters can integrate with external data sources like databases or APIs. Learn more in the [State Management documentation](/docs/internals/state-management).
