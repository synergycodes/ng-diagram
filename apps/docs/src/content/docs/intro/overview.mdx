...
<Aside type="note">
  To set a manual size for nodes, you must explicitly set [`autoSize: false`](/docs/api/types/model/simplenode#autosize) along with the [`size`](/docs/api/types/model/simplenode#size) property. Without
...
- [`selectionMoved`](/docs/api/types/events/selectionmovedevent) - Fired when selected nodes are moved (dragged)
...
- [`selectionRotated`](/docs/api/types/events/selectionrotatedevent) - Fired when selected nodes are rotated
...
- [`nodeResized`](/docs/api/types/events/noderesizedevent) - Fired when a node is resized
