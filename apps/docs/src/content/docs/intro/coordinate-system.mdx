...
- **Positive X**: Move the viewport left
...
- Groups are positioned at their top-left corner
- Child nodes within groups maintain their relative positions
...
```typescript
{
  ...
  position: { x: 0, y: 0 }, // Absolute position (calculated automatically)
  // ... other properties
}
```
...
ngDiagram provides methods to convert between different coordinate systems:

**Client to Flow Position:**

```typescript
// Convert screen coordinates to diagram coordinates
const flowPosition = flowCore.clientToFlowPosition({ x: 150, y: 200 });
```
...
const clientPosition = flowCore.flowToClientPosition({ x: 100, y: 150 });
```
