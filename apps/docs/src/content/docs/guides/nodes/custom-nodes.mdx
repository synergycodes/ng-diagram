...
sidebar:
  order: 3
helpBanner:
  title: Designing advanced node templates?
  body: If your nodes need rich interactions, nested components, or embedded live data, our team has built node templates for many client projects. We are happy to share patterns and trade-offs from that work.
---

import CustomDefaultNode from '@components/angular/nodes/custom-default-node/custom-default-node.astro';
...
You can create a custom node by writing a component that implements the [`NgDiagramNodeTemplate`](/docs/api/types/templates/ngdiagramnodetemplate) interface.
...
If you don't want to start from scratch, you can use the [`NgDiagramBaseNodeTemplateComponent`](/docs/api/components/ngdiagrambasenodetemplatecomponent) component as a way to add custom content into the default node.

Simply wrap your custom content inside the `<ng-diagram-base-node-template>` component.

<CodeSnippet relativePath="nodes/custom-default-node/node/node.component.ts" />
...
To register a custom node, you need to add it to the [`nodeTemplateMap`](/docs/api/types/templates/ngdiagramnodetemplatemap).
...
After registering, you need to pass the map to the [`ng-diagram`](/docs/api/components/ngdiagramcomponent) component.

<CodeSnippet relativePath="nodes/custom-node/diagram.component.ts" sectionId="usage" />

## Setting Node Type

Assigning the [`type`](/docs/api/types/model/simplenode#type) property as `myType` for the node allows you to precisely control which template is used to render each node.
...
By using this approach—creating custom node components that implement [`NgDiagramNodeTemplate`](/docs/api/types/templates/ngdiagramnodetemplate) and receive the node data as input—you gain full access to the Node definition and all its properties.
...
[SimpleNode API Reference →](/docs/api/types/model/simplenode)
