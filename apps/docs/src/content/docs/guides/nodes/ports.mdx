...
<Aside type="tip">
  When you need to read updated measurements immediately after invalidation, wrap both the model update and
  `invalidateMeasurements()` inside a [transaction](/docs/guides/transactions) with [`waitForMeasurements:
  true`](/docs/api/types/middleware/transactionoptions/#waitformeasurements).
</Aside>
