...
this.ngDiagramModelService.updateNodes([{ id: 'node-1', data: { ports: reorderedPorts } }]);
...
<Aside type="tip">
  When you need to read the updated measurements immediately, `await` the call
  <Badge text="since v1.3.0" variant="tip" /> — the returned promise resolves once the re-measurements have settled, so
  the fresh `measuredPorts` are readable on the next line:
...
const ports = this.ngDiagramModelService.getNodeById('node-1')?.measuredPorts;
```

Ignoring the promise is still fine when you only want to trigger the re-measurement.
...
Ports are declared in templates, so you hide them with the `hidden` input. It works independently of the node that owns the port:

```html
<ng-diagram-port id="out" side="right" type="source" [hidden]="!editMode()" />
```

A plain `hidden` attribute without a binding works too. It means hidden, like the native HTML attribute. A hidden port cannot receive a link, and the linking preview does not snap to it. Edges connected to a hidden port stay visible and keep using the port's last measured position.
