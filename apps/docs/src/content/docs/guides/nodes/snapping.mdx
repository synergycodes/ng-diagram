...
Snap drag functionality is disabled by default. The default snap step is set to `{ x: 10, y: 10 }`.
...
const config = {
  snapping: {
    ...
    computeSnapForNodeDrag: (node) => ({ x: 10, y: 10 }),
...
<DragSnapExample />
<br />
<CodeSnippet relativePath="nodes/snapping/drag-snap/diagram.component.ts" sectionId="config" />

## Snap Resize Configuration

Snap resize functionality is enabled by default. The default snap step is set to `{ x: 10, y: 10 }`.
...
const config = {
  snapping: {
    ...
    computeSnapForNodeResize: (node) => ({ x: 10, y: 10 }),
