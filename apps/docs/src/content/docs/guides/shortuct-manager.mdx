...
The **ShortcutManager** handles keyboard input matching in ngDiagram. It supports platform-specific modifiers (Ctrl/Cmd normalization), multiple key bindings per action, and runtime configuration updates.

## Default Shortcuts

| Action                   | Shortcut                                            | Description                           |
| ------------------------ | --------------------------------------------------- | ------------------------------------- |
| `copy`                   | <kbd>Ctrl/Cmd + C</kbd>                             | Copy selected elements                |
| `cut`                    | <kbd>Ctrl/Cmd + X</kbd>                             | Cut selected elements                 |
| `paste`                  | <kbd>Ctrl/Cmd + V</kbd>                             | Paste elements                        |
| `deleteSelection`        | <kbd>Delete</kbd> or <kbd>Backspace</kbd>           | Delete selected elements              |
| `selectAll`              | <kbd>Ctrl/Cmd + A</kbd>                             | Select all elements                   |
| `undo`                   | <kbd>Ctrl/Cmd + Z</kbd>                             | Undo last action                      |
| `redo`                   | <kbd>Ctrl/Cmd + Y</kbd>                             | Redo last undone action               |
| `keyboardMoveSelection*` | <kbd>↑</kbd> <kbd>↓</kbd> <kbd>←</kbd> <kbd>→</kbd> | Move selected elements                |
| `keyboardPan*`           | <kbd>↑</kbd> <kbd>↓</kbd> <kbd>←</kbd> <kbd>→</kbd> | Pan viewport                          |
| `boxSelection`           | <kbd>Shift</kbd> held                               | Enable box selection (pointer events) |
| `preserveSelection`      | <kbd>Ctrl/Cmd</kbd> held                            | Preserve selection (pointer events)   |
...
const config = {
  shortcuts: configureShortcuts([
    ...
    // Multiple bindings: WASD + Arrow keys
    {
...
- **Second parameter:** Base shortcuts (defaults to `DEFAULT_SHORTCUTS`)
...
See [`ShortcutActionName`](/docs/api/types/shortcutactionname/) API reference for all available action names.

**Clipboard:** `copy`, `cut`, `paste`
**Selection:** `deleteSelection`, `selectAll`, `boxSelection`, `preserveSelection`
**Movement:** `keyboardMoveSelectionUp`, `keyboardMoveSelectionDown`, `keyboardMoveSelectionLeft`, `keyboardMoveSelectionRight`
**Panning:** `keyboardPanUp`, `keyboardPanDown`, `keyboardPanLeft`, `keyboardPanRight`
**History:** `undo`, `redo`
