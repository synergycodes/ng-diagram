...
The `preserveSelection` and `boxSelection` actions are **pointer-only** and can only use modifier-only bindings:

```typescript
// ✅ VALID: Pointer-only action with modifiers
{
  actionName: 'preserveSelection',
  bindings: [{ modifiers: { primary: true } }]
}

// ❌ INVALID: Cannot add key bindings to pointer-only actions
{
  actionName: 'preserveSelection',
  bindings: [{ key: 'q', modifiers: { shift: true } }] // TypeScript error!
}
```

All other actions are keyboard actions and can use key-based bindings.
