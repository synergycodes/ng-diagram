...
To display a label on a **default edge**, add a `label` property to the edge's [`data`](/docs/api/types/model/edge/#data). You can also control the label position by setting the `positionOnEdge` data property (defaults to `0.5` — centered).
...
- `--edge-label-border-color-hover` - Label border color while the edge is hovered
- `--edge-label-border-color-selected` - Label border color while the edge is selected
...
### Reusing the Default Label

To give a custom edge the same label look as the default edge, wrap your content in
[`NgDiagramDefaultEdgeLabelComponent`](/docs/api/components/ngdiagramdefaultedgelabelcomponent/) <Badge text="since v1.3.0" variant="tip" />
...
The two elements have separate jobs: `ng-diagram-base-edge-label` places the label along the
edge path and measures it, while `ng-diagram-default-edge-label` only gives the content its look.

The chip keeps the theme colors, responds to the CSS variables listed above, and highlights
its border while the edge is hovered or selected — the same states as in the default edge.
Use `--edge-label-border-color-hover` and `--edge-label-border-color-selected` to change the
highlight colors.
...
Absolute values are clamped to the path length — `'500px'` on a 300px edge resolves to the target end, and `'-500px'` resolves to the source end.
...
ngDiagram automatically measures label dimensions using a `ResizeObserver`. Measurements update automatically when:

- A label's content changes size
- The [`positionOnEdge`](/docs/api/types/model/edgelabel/#positiononedge) property is updated

If you alter a label's layout through CSS in a way that doesn't trigger a size change, call [`invalidateMeasurements()`](/docs/api/services/ngdiagramservice/#invalidatemeasurements) <Badge text="since v1.2.3" variant="tip" /> to force re-measurement. `await` it <Badge text="since v1.3.0" variant="tip" /> when you need the fresh `measuredLabels` right away:

```typescript
await this.ngDiagramService.invalidateMeasurements({
  edges: [{ edgeId: 'edge-1' }],
});
```

See [Port Measurement](/docs/guides/nodes/ports#port-measurement) for more details on when this is needed.
...
Labels are declared in templates, so they are hidden declaratively through the `hidden` input — independently of their owning edge:
...
The static attribute form works too — a bare `hidden` attribute means hidden, matching native HTML semantics. A hidden label stays mounted, keeps its measured geometry, and never blocks initialization.

See [Conditional Visibility](/docs/guides/conditional-visibility) for the full semantics and how hiding interacts with measurement.
