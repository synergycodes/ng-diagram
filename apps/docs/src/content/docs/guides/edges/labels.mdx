...
Labels are declared in templates, so you hide them with the `hidden` input. It works independently of the edge that owns the label:
...
A plain `hidden` attribute without a binding works too. It means hidden, like the native HTML attribute. A hidden label stays in the DOM, keeps its measured size, and never blocks initialization.

See [Conditional Visibility](/docs/guides/conditional-visibility) for the full rules and how hiding works with measurement.
