...
<Aside type="tip">
  Operations are **grouped by type** and applied in this fixed order:

Add nodes → Add edges → Remove edges → Remove nodes → Update nodes → Update edges → Merge metadata

This means call order inside transactions is **not preserved**. For example: `addNodes([node1])` → `addEdges([edge1])` → `addNodes([node2])` will execute both node additions together.
