...
All available shortcut actions with their default key bindings. See [`ShortcutActionName`](/docs/api/types/configuration/shortcuts/shortcutactionname) for the complete type reference.

| Action                       | Default Shortcut                          | Description                                                                                                         |
| ---------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `boxSelection`               | <kbd>Shift</kbd> held                     | Enable box selection mode (pointer only)                                                                            |
| `cancelInteraction`          | <kbd>Escape</kbd>                         | Cancel the in-progress interaction (linking, drag, resize, rotate, pan) <Badge text="since v1.3.0" variant="tip" /> |
...
### Arrow Keys: Move vs Pan

Each arrow key is bound to two actions: a `keyboardMoveSelection` action and a `keyboardPan` action (see the table
above). Exactly one of them handles each key press:

- **The selection moves** when node dragging is enabled
  ([`nodeDraggingEnabled`](/docs/api/types/configuration/flowconfig#nodedraggingenabled)) and at least one selected
  node is visible and draggable.
- **The viewport pans** in every other case: nothing is selected, `nodeDraggingEnabled` is `false`, or the selection
  contains only hidden or `draggable: false` nodes. Panning requires
  [`viewportPanningEnabled`](/docs/api/types/configuration/flowconfig#viewportpanningenabled).

A selection that cannot move never blocks the arrow keys.

## Focus and Tab Order

Shortcuts only work when focus is inside the diagram. The diagram takes focus when the user clicks it, and it is
also part of the page's Tab order, so the user can reach it with the <kbd>Tab</kbd> key.

The diagram adds two Tab stops of its own: the diagram container and the watermark link.

### Removing the Diagram from the Tab Order <Badge text="since v1.3.1" variant="tip" />

If your application manages the focus on its own — for example with a roving `tabindex` on the nodes — set
[`tabbable`](/docs/api/components/ngdiagramcomponent/#tabbable) to `false`. Both Tab stops are then removed from the Tab order:

```html
<ng-diagram [model]="model" [tabbable]="false" />
```

- Both elements stay clickable and keep working with screen readers. Only the Tab stop is removed.
- Shortcuts keep working once focus is inside the diagram, for example after a click.
- Focusable content in your own node and edge templates is not affected. It keeps its own Tab stops, so
  the application stays in control of them.

## Basic Usage

To override, disable, or extend default shortcuts, use the [`configureShortcuts()`](/docs/api/utilities/configureshortcuts) helper:
...
[`configureShortcuts(customShortcuts, baseShortcuts?)`](/docs/api/utilities/configureshortcuts/) merges your custom shortcuts with defaults:
...
See [`ShortcutDefinition`](/docs/api/types/configuration/shortcuts/shortcutdefinition/) API reference.
...
such as holding <kbd>Shift</kbd> for box selection or holding <kbd>Ctrl/Cmd</kbd> and using mouse wheel for zoom in and zoom out.
