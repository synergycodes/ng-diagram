...
Shortcuts only work when the focus is inside the diagram. The diagram takes the focus when the user clicks it, and it is
...
[`tabbable`](/docs/api/components/ngdiagramcomponent/#tabbable) to `false`. Both Tab stops then leave the Tab order:
...
- Shortcuts keep working once the focus is inside the diagram, for example after a click.
