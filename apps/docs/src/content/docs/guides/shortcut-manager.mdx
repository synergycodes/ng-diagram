...
The arrow keys are bound to both `keyboardMoveSelection*` and `keyboardPan*`, and exactly one of the two actions
claims each key press. The keys move the selection when it would actually move — node dragging is enabled
([`nodeDraggingEnabled`](/docs/api/types/configuration/flowconfig)) and at least one selected node is visible and
draggable. In every other case they pan the viewport (as long as `viewportPanningEnabled` is set): with no selection,
with `nodeDraggingEnabled: false`, or when the selection contains only hidden or `draggable: false` nodes. A selection
that cannot be moved never swallows the arrow keys.
