...
If you have a custom [`ModelAdapter`](/docs/api/types/model/modeladapter/) implementation (e.g., backed by localStorage, NgRx, or an external store), use [`initializeModelAdapter`](/docs/api/utilities/initializemodeladapter) instead. It strips stale runtime properties and assigns fresh internal IDs to all nodes and edges in the adapter.
