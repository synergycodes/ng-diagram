...
<Aside type="note">
  When using `initializeModel` inside a reactive context, you must pass the `injector` parameter explicitly since reactive contexts do not provide an Angular injection context.
</Aside>

<Aside type="caution">
  Each signal change triggers a full [reinitialization cycle](#initialization-process) (measurement, layout, event emission). Keep this in mind for frequently changing data sources.
</Aside>
...
<Aside type="tip">
  When using the [reactive approach](#using-in-reactive-contexts), `computed` handles this automatically — if the signal value is the same reference, the model won't reinitialize.
</Aside>
