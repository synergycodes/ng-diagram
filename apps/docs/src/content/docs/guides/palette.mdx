...
The core building block of the palette is the [`<ng-diagram-palette-item>`](/docs/api/components/ngdiagrampaletteitemcomponent) component.
...
A palette item is defined by an [`NgDiagramPaletteItem`](/docs/api/types/palette/ngdiagrampaletteitem/):
...
When using [`<ng-diagram-palette-item>`](/docs/api/components/ngdiagrampaletteitemcomponent), you can pass any HTML or Angular template as its content to represent the item in the palette.
...
To define the preview, wrap your content in [`<ng-diagram-palette-item-preview>`](/docs/api/components/ngdiagrampaletteitempreviewcomponent) inside [`<ng-diagram-palette-item>`](/docs/api/components/ngdiagrampaletteitemcomponent).

The preview itself can be any HTML or Angular component.

The preview component holds a hidden, natural-size copy of its content. The component clips this copy internally, so it takes no space in your layout and never shows up on the page. You can place [`<ng-diagram-palette-item-preview>`](/docs/api/components/ngdiagrampaletteitempreviewcomponent) anywhere inside the palette item and lay out the palette like any other panel — no wrapper with `position: relative` or `overflow: hidden` is needed.

During a drag, the copy is scaled to the current zoom of the diagram (up to 3x) and shown under the cursor, so the drag image matches the node that will be dropped.

import Palette from '@examples/palette/palette.astro';

<Palette />

<CodeViewer dirName="examples/palette" />
