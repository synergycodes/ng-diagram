...
<Aside type="caution" title="Reading state right after a change">
  The `nodes()`, `edges()` and `metadata()` signals refresh with Angular's change detection.
  Right after an `await` — and inside diagram event handlers — they can still return the
  previous state. To read the state immediately, use the getter methods (`getNodeById()`,
  `getEdgeById()`, `getConnectedEdges()`, …) or the model itself via `getModel()` — these are
  always synchronous with the applied changes. Use the signals in reactive contexts: templates,
  `computed()`, and `effect()`.
</Aside>
