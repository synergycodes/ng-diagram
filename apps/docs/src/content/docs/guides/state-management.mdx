...
### Awaiting Changes <Badge text="since v1.3.0" variant="tip" />

Every mutating service method returns a `Promise<void>` that resolves once the change has been
applied to the model — the next line of code reads the updated state:

```typescript
await this.modelService.deleteNodes(['node-1']);
// The model is already updated here — safe to act on it:
this.viewportService.zoomToFit();
```

Awaiting is optional — a call whose promise you ignore applies the change all the same. Await
when the next step depends on the change being applied: sequencing operations, reading the
model right after a mutation, or reacting to a rejected update (a failing
[middleware](/docs/guides/middlewares/) rejects the returned promise). In projects using the
`@typescript-eslint/no-floating-promises` lint rule, mark intentional fire-and-forget calls
with the `void` operator: `void this.modelService.addNodes([node]);`.

Inside a [transaction](/docs/guides/transactions/) the promise resolves right away and the
change is applied when the transaction commits.

<Aside type="caution" title="Reading state right after a change">
  The `nodes()`, `edges()` and `metadata()` signals refresh with Angular's change detection. Right after an `await` —
  and inside diagram event handlers — they can still return the previous state. To read the state immediately, use the
  getter methods (`getNodeById()`, `getEdgeById()`, `getConnectedEdges()`, …) or the model itself via `getModel()` —
  these are always synchronous with the applied changes. Use the signals in reactive contexts: templates, `computed()`,
  and `effect()`.
</Aside>
...
<Aside type="tip" title="Waiting for measurements">
  When you need to perform viewport operations (like
  [`zoomToFit()`](/docs/api/services/ngdiagramviewportservice/#zoomtofit) or
  [`centerOnNode()`](/docs/api/services/ngdiagramviewportservice/#centeronnode)) immediately after adding or modifying
  elements, pass `{ waitForMeasurements: true }` to the method itself (see the
  [per-method option](/docs/guides/transactions/#per-method-option)) or use the
  [`waitForMeasurements`](/docs/guides/transactions/#waitformeasurements) transaction option to ensure the elements are
  measured before the viewport adjusts.
</Aside>
...
Deleting a node also removes the edges connected to it. Deleting a group also deletes all of its
descendants (children, grandchildren, and so on). This is the same behavior as deleting the selection.
...
```typescript
// Get element by ID
const node = this.modelService.getNodeById<NodeData>('node-1');
const edge = this.modelService.getEdgeById<EdgeData>('edge-1');

// Find nearest node to a point
const nearestNode = this.modelService.getNearestNodeInRange<NodeData>(
  { x: 150, y: 150 },
  50 // range in pixels
);
...
const nodesInRange = this.modelService.getNodesInRange<NodeData>({ x: 150, y: 150 }, 100);
```
...
Use [`initializeModelAdapter`](/docs/api/utilities/initializemodeladapter) to prepare a custom adapter for use with ngDiagram.\
...
```typescript
// Sketch — a real adapter implements every ModelAdapter member;
// see the Custom Model example for a complete implementation.
class NgRxModelAdapter implements ModelAdapter {
  ...
  // ...remaining ModelAdapter members
}

// Wire up the custom adapter with initializeModelAdapter
