...
To remove a node from a group, the method depends on the `allowGroupAutoResize` setting:

- **When `allowGroupAutoResize` is disabled:**  
   Simply drag the node out of the group. The node will be removed from the group automatically.
- **When `allowGroupAutoResize` is enabled (default):**  
   Hold the `SHIFT` key while dragging the node out of the group. This ensures the node is removed from the group, as dragging alone will trigger group autoresize instead.

For more details on group autoresize, see the [Group Resizing via diagram config](/docs/internals/nodes/groups/#resizing-configuration).
...
- **Manual Resize:** Set `resizable: true` and optionally provide a `size` property.
- **Autoresize:** With `allowGroupAutoResize: true` (default) set in a diagram configuration, the group automatically fits its children.
...
- **allowGroupAutoResize:**  
   When `true` (default), groups automatically resize to fit their children.  
   When `false`, groups do not resize automatically, and you must manually adjust their size.
...
See [Custom Groups](./custom-groups) for information on creating your own group templates, registering them, and handling advanced behaviors.
