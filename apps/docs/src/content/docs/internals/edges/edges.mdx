...
Ports are connection points on nodes where edges can start or end. Each port has an ID unique within its node. These IDs (along with their parent node IDs) are used to specify the exact position where the edge should connect.
...
Edges can be routed using three built-in algorithms:

- `straight`: Draws a straight line between the nodes
- `orthogonal`: Draws a series of horizontal and vertical lines
- `curved`: Draws a smooth curve between the nodes
