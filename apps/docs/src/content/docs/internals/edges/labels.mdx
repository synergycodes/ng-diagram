...
Based on the usage of the `BaseEdgeLabelComponent`, NgDiagram tracks the label's position and size, ensuring they remain aligned with the edge's path. The `positionOnEdge` property determines where the label appears along the edge path. It can be set to any value from `0` to `1`, where `0.5` is the center of the edge. Labels automatically follow the edge path, regardless of the used [routing algorithm](/internals/edges/routing).
