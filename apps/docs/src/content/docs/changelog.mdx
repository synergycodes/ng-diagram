...
- `waitForMeasurements` option on content-creating and updating service methods — `addNodes`, `addEdges`, `updateNode`, `updateNodes`, `updateEdge`, `updateEdges` on `NgDiagramModelService` and `resizeNode` on `NgDiagramNodeService` accept `options?: { waitForMeasurements?: boolean }`; when set, the returned promise resolves only after measurements triggered by the change have completed, so `zoomToFit()`/`centerOnNode()` can be called immediately after ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))

### Fixed

- **Drag re-entrancy hardening** — the node-drag handler no longer loses or double-applies movement when command emits suspend on asynchronous middleware, and a fast re-drag no longer gets its state clobbered by the previous drag's asynchronous cleanup ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))
