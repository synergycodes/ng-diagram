...
- **Port and edge-label measurements no longer get lost during rapid re-rendering** — fast mount/unmount cycles (e.g. virtualization while panning) could apply port and label updates out of order or drop them entirely when one update failed, leaving stale port positions or labels ([#TBD](https://github.com/synergycodes/ng-diagram/pull/TBD))
