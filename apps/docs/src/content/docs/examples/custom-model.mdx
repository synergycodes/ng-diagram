...
Use [`initializeModelAdapter`](/docs/api/utilities/initializemodeladapter/) to prepare a custom adapter for use with ng-diagram.
...
A custom `ModelAdapter` makes your storage layer the single source of truth — every change goes straight to it. If you only need save/load buttons on top of the default model, snapshotting with `toJSON()` is much simpler — see the [Save Persistence example](/docs/examples/save-state/).

**Learn more:** [State Management guide →](/docs/guides/state-management/)
