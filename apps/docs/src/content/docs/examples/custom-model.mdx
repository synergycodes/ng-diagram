...
<CustomModel />
<ExampleViewer dirName="examples/custom-model" />

## Additional Explanation

### Custom Model Overview

The `NgDiagramComponent` accepts any object, via the `model` input property (e.g. `[model]="customModelAdapter"`), that implements the `ModelAdapter` interface.
...
The `ModelAdapter` interface defines the contract that any model implementation must fulfill. You can find the complete interface documentation in the [API reference](/docs/api/types/modeladapter/). The key methods include:
