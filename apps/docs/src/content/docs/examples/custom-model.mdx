...
- **Memory Efficient**: No local copies of data are maintained in memory
...
- **Data modification**: `updateNodes`, `updateEdges`, `setMetadata`
...
The `LocalStorageModelAdapter` implements the `ModelAdapter` interface to provide persistent storage:

<CustomModel />

<ExampleViewer dirName="custom-model" />

This example demonstrates a custom model adapter that persists diagram data to localStorage. The data will survive page refreshes and browser sessions.
