...
// state, not the model).
//
// Exported so `cancelLinking` can reuse the exact same pass; both callers clear
// the linking state afterwards via `clearLinkingForGesture` (see the `finally`
// in `finishLinking`), never inside this helper.
