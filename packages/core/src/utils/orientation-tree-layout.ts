...
export const horizontalTreeLayout = (
  ...
  children.forEach((child, i) => {
    ...
    const childX = sign === 1 ? offsetX + parentWidth + config.levelGap : offsetX - (childWidth + config.levelGap);
  ...
  if (!isHorizontal && children.length > 0) {
    parentNode.position.x = (firstChild.position.x + lastChild.position.x) / 2;
    parentNode.position.y = offsetY;
    return offsetY + maxChildrenHeight + parentHeight + config.levelGap;
  }

  parentNode.position.y = !isHorizontal ? offsetY : (topOffsetY + bottomOffsetY - parentHeight) / 2;
  parentNode.position.x = offsetX;
...
export const verticalTreeLayout = (
  ...
  // Offsets for children
  let childOffsetX = offsetX + (!isVertical ? parentWidth + config.levelGap : 0);
  ...
  children.forEach((child, i) => {
    const { width: childWidth = 0, height: childHeight = 0 } = child.size || {};
    const childY = sign === 1 ? offsetY + parentHeight + config.levelGap : offsetY - (childHeight + config.levelGap);

    const childX = isVertical ?
  ...
  parentNode.position.x = !isVertical ? offsetX : (topOffsetX + bottomOffsetX - parentWidth) / 2;
  // Return updated offset
  const maxChildrenWidth = Math.max(...children.map((child) => child.size?.width || 0));

  if (!isVertical && children.length > 0) {
    parentNode.position.x = offsetX;
    parentNode.position.y = (firstChild.position.y + lastChild.position.y) / 2;
    return offsetX + maxChildrenWidth + parentWidth + config.levelGap
  }

  return !isVertical ? offsetX + maxChildrenWidth + parentWidth + config.levelGap : currentOffsetX;
};
