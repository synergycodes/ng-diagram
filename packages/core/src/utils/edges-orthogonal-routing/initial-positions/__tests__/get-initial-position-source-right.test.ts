...
import { getInitialPositionSourceRight } from '../get-initial-position-source-right';

describe('getInitialPositionSourceRight', () => {
  ...
  describe('when targetPortSide is "right"', () => {
    it('returns vertical path when source is to the right of target', () => {
      ...
      const points = getInitialPositionSourceRight('right', xySource, xyTarget, xyCenter);
  ...
  describe('when targetPortSide is "top"', () => {
    it('returns direct path when source is to the left and above target', () => {
      ...
      const xyTarget = { x: 60, y: 40 };
      const points = getInitialPositionSourceRight('top', xySource, xyTarget, xyCenter);
  ...
  describe('when targetPortSide is "bottom"', () => {
    it('returns three-point path when source is to the left and above target', () => {
      ...
      const points = getInitialPositionSourceRight('bottom', xySource, xyTarget, xyCenter);
      expect(points).toEqual([
        { x: 50, y: 20 },
        { x: 50, y: 80 },
        { x: 60, y: 80 },
  ...
  describe('when targetPortSide is "left"', () => {
    it('returns horizontal path when source is far to the left of target', () => {
      ...
      const xyTarget = { x: 60, y: 40 };
      const points = getInitialPositionSourceRight('left', xySource, xyTarget, xyCenter);
      expect(points).toEqual([
        { x: 50, y: 40 },
        { x: 50, y: 40 },
      ]);
