...
export const getSourceTarget = (edge: Edge, nodesMap: Map<string, Node>) => {
  ...
  const sourcePoint = getPoint(edge.source, 'left', edge.sourcePort, edge.sourcePosition);
  const targetPoint = getPoint(edge.target, 'right', edge.targetPort, edge.targetPosition);
