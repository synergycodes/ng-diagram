...
describe('FlowCore', () => {
  ...
  describe('transaction mechanism', () => {
    it('should queue updates during an active transaction and apply them atomically on stopTransaction', async () => {
      flowCore.startTransaction('updateNodes');
      // Queue two updates
      await flowCore.applyUpdate({ nodesToAdd: [mockNode] }, 'addNodes');
      await flowCore.applyUpdate({ edgesToAdd: [mockEdge] }, 'addEdges');
      // Middleware should not be called yet
      ...
      // Middleware should be called once with merged updates
      expect(mockMiddlewareManager.execute).toHaveBeenCalledTimes(1);
      const callArgs = mockMiddlewareManager.execute.mock.calls[0];
      expect(callArgs[1].nodesToAdd).toContain(mockNode);
      expect(callArgs[1].edgesToAdd).toContain(mockEdge);
      // State should be set
      expect(mockModelAdapter.setNodes).toHaveBeenCalledWith([mockNode]);
      expect(mockModelAdapter.setEdges).toHaveBeenCalledWith([mockEdge]);
    });

    it('should throw if startTransaction is called twice', async () => {
      flowCore.startTransaction('updateNodes');
      // Best practice: check for specific error message
      expect(() => flowCore.startTransaction('updateNodes')).toThrow(
        'A transaction is already active. Only one transaction can be active at a time.'
      );
      // Clean up
      await flowCore.stopTransaction();
    });

    it('should throw if stopTransaction is called without an active transaction', async () => {
      // Best practice: use try/catch for async error assertion
      await expect(async () => {
        await flowCore.stopTransaction();
      }).rejects.toThrow('No active transaction to stop.');
    });

    it('should not call middleware if no updates were queued in transaction', async () => {
      flowCore.startTransaction('updateNodes');
      mockMiddlewareManager.execute.mockClear();
      await flowCore.stopTransaction();
      expect(mockMiddlewareManager.execute).not.toHaveBeenCalled();
    });
  });
