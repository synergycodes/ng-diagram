...
import { __NEW__InputActionHandler } from '../__new__types';
...
export const selectActionHandler: __NEW__InputActionHandler = {
  action: (event, flowCore) => {
    console.log('alalalalala, select alalalala', event);
  },
  predicate: (event, flowCore) => {
    return true;
    // // Type guard to ensure we have a pointer event
    // if (!event || event.source !== 'pointer') return false;

    // // Check if the event is a select event and not on a resize handle
    // return event.name === 'select' && !event.target?.isResizeHandle;
  },
};
